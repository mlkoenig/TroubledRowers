...
public class BodyEditorLoader {
    ...
    /**
     * Attach fixture with width and height
     * @param body
     * @param name
     * @param fd
     * @param scale
    ...
    public void attachFixture(Body body, String name, FixtureDef fd, Sprite sprite, float scale) {
        RigidBodyModel rbModel = model.rigidBodies.get(name);
        if (rbModel == null) throw new RuntimeException("Name '" + name + "' was not found.");
        ...
        for (int i=0, n=rbModel.polygons.size(); i<n; i++) {
            ...
            for (int ii=0, nn=vertices.length; ii<nn; ii++) {
                vertices[ii] = newVec().set(transform(polygon.vertices.get(ii), sprite)).scl(scale);
                vertices[ii].sub(origin);
            }
    ...
    private Vector2 transform(Vector2 v, Sprite sprite){
        return transform(v.x, v.y, sprite);
    }

    private Vector2 transform(float x, float y, Sprite sprite){
        return new Vector2(transformX(x, sprite), transformY(y, sprite));
    }

    private Vector2 transform(Vector2 v, float origWidth, float origHeight, float width, float height) {
        return transform(v.x, v.y, origWidth, origHeight, width, height);
    }

    private Vector2 transform(float x, float y, float origWidth, float origHeight, float width, float height) {
        return new Vector2(transformX(x, width), transformY(y, origWidth, origHeight, height));
    }

    /**
     * Transforms the x vertices of the hitbox / PolygonShape to the correct sizes.
     * @param x
     * @return
     */
    private float transformX(float x, Sprite sprite){
        return transformX(x, sprite.getWidth());
    }
    ...
    /**
     * Transforms the y vertices of the hitbox / PolygonShape to the correct sizes.
     * @param y
     * @return
     */
    private float transformY(float y, Sprite sprite){
        return transformY(y, sprite.getRegionWidth(), sprite.getRegionHeight(), sprite.getHeight());
    }
