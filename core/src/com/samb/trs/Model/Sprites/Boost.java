...
public class Boost extends LocationBody implements Drawable {
    ...
    public Boost(MainController mainController, TextureRegions regions, World world, Viewport viewport, float px, float py, float radius) {
    ...
    public void defineFixtures() {
        ...
        fixtureDef.filter.categoryBits = Constants.Collision.BOOST_BIT;
    ...
    public void removeWhenOutOfBounds(Viewport viewport) {
        if (getY() < -HEIGHT / 2f - getHeight() + viewport.getCamera().position.y || getY() > 3f * HEIGHT + getHeight() + viewport.getCamera().position.y) {
